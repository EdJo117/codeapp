...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("International", icon = icon("database"), tabName = "rdb", startExpanded = TRUE,
               
               menuSubItem(icon = NULL,
                           selectizeInput("var", "Perimètre",
                                          choices = list('AL','BENL','CN','COMEXT',
                                                         'DM','EM','FI','FITAUX',
                                                         'INV','IT','JP','OIL','SYN','UK','US','ZE'), multiple = F,
                                          width = '98%') )
      ),
      menuItem("France", icon = icon("database"), tabName = "rdb1", startExpanded = TRUE,
               
               menuSubItem(icon = NULL,
                           selectizeInput("var", "Perimètre",
                                          choices = list('CONSO','ES',
                                                         'EMPL','PIBFR','PROD',
                                                         'REV'), multiple = F,
                                          width = '98%') )
      )
